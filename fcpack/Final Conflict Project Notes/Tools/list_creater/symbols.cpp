/*
Final Conflict: Symbol Converter
*/
...
void Process_Text()
{
	...
	list_entries = fopen( "lists.txt", "w" );
	...
	while( fgets( line, sizeof( line ), list ) ) {
		...
		if( *pText == '#' && strlen( line ) == 1 ) {
			...
			if( table_number == 1 ) {
				printf( "  [t1e.asm = CLASS]" );
				fprintf( list_entries, "#define CLASS $ae00+$%x\n", table_offset - table_length );
			}
			if( table_number == 2 ) {
				printf( "  [t1e.asm = ITEMS]" );
				fprintf( list_entries, "#define ITEMS $ae00+$%x\n", table_offset - table_length );
			...
			if( table_number == 3 ) {
				printf( "  [t1e.asm = SPELL]" );
				fprintf( list_entries, "#define SPELL $b500+$%x\n", table_offset - table_length );
			}

			if( table_number == 4 ) {
				printf( "  [t1f.asm = NAME_PTR, NAME_LEN]" );
				fprintf( list_entries, "#define NAME_PTR $b500+$%x\n", table_offset - table_length );
				fprintf( list_entries, "#define NAME_LEN $%x\n", table_length );
			}

			if( table_number == 5 ) {
				printf( "  [t1e_3.asm = MONSTER]" );
				fprintf( list_entries, "#define MONSTER $b500+$%x\n", table_offset - table_length );
			}

			if( table_number == 6 ) {
				printf( "  [t7a_3.asm = DICT]" );
				fprintf( list_entries, "#define DICT $b500+$%x\n", table_offset - table_length );
			}
